...
> **为什么需要它？** 两类应用无法使用输入实时翻译：
>
> 1. **微信**：聊天窗口不向 macOS 系统开放无障碍（Accessibility）接口，任何实时翻译工具都无法读取其中正在输入的文字——这是应用自身的限制，不是 Pico 的问题。
> 2. **谷歌系软件（Chrome、Edge 等 Chromium 内核应用）**：这类应用的界面文本不对系统稳定开放，实时翻译时灵时不灵，不能依赖。
>
> 因此 Pico 专门提供了剪贴板翻译，作为这两类应用的稳定方案：**正常选中文字按 ⌘C 复制，再按 ⌥⇧V 即可出译文**；支持无障碍的应用（备忘录、TextEdit、多数原生办公软件等）则照常自动实时翻译，无需任何操作。

- 任意应用复制文字后按 **⌥⇧V**（可改），译文浮窗立即弹出
- 自动监听模式：复制包含源语言的内容即自动翻译，代码与链接不干扰
- 黑名单应用（设置 → 隐私 → 排除的应用）保持完全静音，连剪贴板自动翻译也不会触发

**悬浮窗**
- 整卡拖动 + 位置记忆，新译文跟随你拖到的位置
- 自动避开当前输入框，不遮挡正在打的字
- 透明度 0%–70% 实时调节，遮挡内容可直接透视
...
不同应用对 macOS 无障碍 API 的支持程度不同：备忘录、TextEdit 和多数原生应用可以实时翻译；**微信、Chrome/Edge 等谷歌系 Chromium 应用、WPS** 的界面文本不向系统稳定开放，实时翻译无法支持或不可靠，这些应用请使用剪贴板翻译（复制 + ⌥⇧V，或开启自动监听）；密码输入框永远跳过，不会被读取。
